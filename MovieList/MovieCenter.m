...
- (BOOL)openDB {
    ...
    if (existFile == NO) {
        const char *creatSQL = "CREATE TABLE IF NOT EXISTS MOVIE (TITLE TEXT, ACTOR TEXT)";
        char *errorMsg;
...
- (NSInteger)getNumberOfActorsInMovie:(NSInteger)movieIndex {
    NSString *queryStr = [NSString stringWithFormat:@"SELECT count(rowid)  FROM MOVIE where rowid=%d", (int)movieIndex];
    sqlite3_stmt *stmt;
    int ret = sqlite3_prepare_v2(db, [queryStr UTF8String], -1, &stmt, NULL);
...
- (NSString *)getNameOfActorAtIndex:(NSInteger)index inMovie:(NSInteger)movieIndex {
    NSString *queryStr = [NSString stringWithFormat:@"SELECT rowid, title FROM MOVIE limit %d, 1", (int)movieIndex];
    sqlite3_stmt *stmt;
    int ret = sqlite3_prepare_v2(db, [queryStr UTF8String], -1, &stmt, NULL);
    ...
    while (SQLITE_ROW == sqlite3_step(stmt)) {
        char *title = (char *)sqlite3_column_text(stmt, 1);
        actorname = [NSString stringWithCString:title encoding:NSUTF8StringEncoding];
...
- (NSInteger)addActorWithName:(NSString *)name inMovie:(NSInteger)movieIndex {
    NSString *sql = [NSString stringWithFormat:@"INSERT INTO MOVIE (actor) VALUES ('%@')", name];
    NSLog(@"sql : %@", sql);
